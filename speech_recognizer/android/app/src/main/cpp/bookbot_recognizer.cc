...
BookbotRecognizer::BookbotRecognizer(BookbotModel *model, float sample_frequency) : model_(model), sample_frequency_(sample_frequency) {
...
BookbotRecognizer::BookbotRecognizer(BookbotModel *model, float sample_frequency, char const *grammar) : model_(model), sample_frequency_(sample_frequency)
{
    KALDI_LOG << "creating recognizer with sample frequency " << sample_frequency;
...
BookbotRecognizer::~BookbotRecognizer() {
    delete decoder_;
    ...
    model_->Unref();
    KALDI_LOG << "Freed recognizer";
}

void BookbotRecognizer::InitState()
{
    frame_offset_ = 0;
...
void BookbotRecognizer::InitRescoring()
{
...
void BookbotRecognizer::CleanUp()
{
    delete silence_weighting_;
    silence_weighting_ = new kaldi::OnlineSilenceWeighting(*model_->trans_model_, model_->feature_info_.silence_weighting_config, 3);
...
void BookbotRecognizer::UpdateSilenceWeights()
{
...
void BookbotRecognizer::SetMaxAlternatives(int max_alternatives)
{
    max_alternatives_ = max_alternatives;
}

void BookbotRecognizer::SetWords(bool words)
{
    words_ = words;
}


bool BookbotRecognizer::AcceptWaveform(const char *data, int len)
{
...
bool BookbotRecognizer::AcceptWaveform(const short *sdata, int len)
{
    Vector<BaseFloat> wave;
...
bool BookbotRecognizer::AcceptWaveform(const float *fdata, int len)
{
    Vector<BaseFloat> wave;
    wave.Resize(len, kUndefined);
    for (int i = 0; i < len; i++)
...
bool BookbotRecognizer::AcceptWaveform(Vector<BaseFloat> &wdata)
{
...
const char *BookbotRecognizer::MbrResult(CompactLattice &rlat)
{
...
const char *BookbotRecognizer::NbestResult(CompactLattice &clat)
{
    Lattice lat;
    Lattice nbest_lat;
...
const char* BookbotRecognizer::GetResult()
{
...
const char* BookbotRecognizer::FinalResult()
{
    if (state_ != RECOGNIZER_RUNNING) {
        return StoreEmptyReturn();
    }
...
const char *BookbotRecognizer::StoreEmptyReturn()
{
...
const char *BookbotRecognizer::StoreReturn(const string &res)
{
    last_result_ = res;
    return last_result_.c_str();
}
