...
BookbotModel::BookbotModel(const char *model_path) : model_path_str_(model_path) {
...
void BookbotModel::ReadDataFiles()
{
    struct stat buffer;
    ...
    //feature_info_.silence_weighting_config.silence_weight = 2;
...
int BookbotModel::FindWord(const char *word)
{
    if (!word_syms_)
        return -1;
