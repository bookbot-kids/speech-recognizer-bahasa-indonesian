///
/// this has all been copied (mostly verbtaim) from book_extensions.dart
/// ideally this should be refactored into a standalone/shared TextCleaner class
///
///
class UKUS {
  static final ukToUS = {
...
class TextCleaner {
  String inCaps(s) => '${s[0].toUpperCase()}${s.substring(1)}';

  String allInCaps(s) => s.toUpperCase();

  String capitalizeFirstofEach(s) =>
      s.split(' ').map((str) => str.inCaps).join(' ');
  ...
  Iterable<String> normalize(String word) sync* {
    ...
    // Do Americanisation or UKisation
    final String nonPossesive =
        word.replaceAll("'s", '').replaceAll(nonAlphaRegEx, '').toLowerCase();

    for (final spellingMap in [UKUS.usToUK, UKUS.ukToUS]) {
      if (spellingMap.containsKey(nonPossesive)) {
        final mapped = spellingMap[nonPossesive] ?? '';
        // Word is capitalised
        final firstLetter = normalisedWithCap.substring(0, 1);
        if (firstLetter == firstLetter.toUpperCase()) {
          word = word.replaceAll(capitalizeFirstofEach(nonPossesive),
              capitalizeFirstofEach(mapped));
        } else {
          word = word.replaceAll(nonPossesive, mapped);
        }

        // Word is possesive
        if (nonPossesive != normalised) {
          normalised = mapped + "'s";
        } else {
          normalised = mapped;
        }
